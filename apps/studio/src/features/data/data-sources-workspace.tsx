  ...
  return (
    <div className="ec-data-source-inspector-content">
      ...
      <section>
        ...
        <p>{source.kind === 'internal' ? 'No requiere gateway para datos locales.' : 'Requiere gateway si el endpoint necesita secretos.'}</p>
      </section>
      <section>
        ...
        {compatibility.length > 0 ? <ul>{compatibility.map((item, index) => <li key={`${index}-${item.message}`}>{item.message}</li>)}</ul> : null}
      </section>
      <section>
        <span>Entornos</span>
...
export function DataSourcesWorkspace() {
  ...
  if (snapshot.state === 'loading' && snapshot.sources.length === 0) {
    return <section className="ec-data-sources-workspace" data-data-sources-workspace><p role="status">Cargando fuentes de datos…</p></section>;
  }
    ...
    return (
      <section className="ec-data-sources-workspace" data-data-sources-workspace>
        <div className="ec-data-sources-empty" role="alert">
          ...
          <Button size="sm" onClick={() => void dataSourceWorkspaceRuntime.load()}>Reintentar</Button>
        </div>
      </section>
    );
  }
    ...
    <section className="ec-data-sources-workspace" data-data-sources-workspace aria-labelledby="ec-data-sources-title">
            ...
            <div role="listbox" aria-label="Fuentes del proyecto">
              {filtered.map((source) => {
                ...
                return (
                  ...
                  >
                    <span aria-hidden="true"><DataSourceIcon /></span>
                    <span><strong>{source.label}</strong><small>{sourceKindLabel(source.kind)} · {source.adapterId}</small></span>
            ...
            {selected ? (
              <>
                <div className="ec-data-source-detail-heading">
                  ...
                  <div className="ec-data-source-detail-title"><p>{sourceKindLabel(selected.kind)}</p><h3>{selected.label}</h3><small>{selected.key}</small></div>
                  <Button className="ec-data-source-inspector-trigger" size="sm" variant="outline" onClick={() => setInspectorOpen(true)}>Seguridad y compatibilidad</Button>
                </div>
                ...
                <section className="ec-data-source-section" aria-labelledby="ec-data-source-summary">
                  ...
                  <div className="ec-data-source-detail-grid">
                    ...
                    <section><span>Estado</span><strong>{adapterRegistered ? 'Registrado' : 'No registrado'}</strong></section>
                    <section><span>Discovery</span><strong>{selected.schemaDiscovery}</strong></section>
                    <section><span>Actualización</span><strong>{dataSourceWorkspaceRuntime.sourceUpdatedAt(selected.id) ?? 'Sin fecha'}</strong></section>
                  </div>
                ...
                <section className="ec-data-source-section" aria-labelledby="ec-data-source-config">
                  ...
                  <small>{environmentEnabled ? 'Este entorno está habilitado.' : 'Esta fuente está deshabilitada en el entorno seleccionado.'}</small>
                </section>
                ...
                <section className="ec-data-source-section" aria-labelledby="ec-data-source-auth">
                  ...
                  <div className="ec-data-source-auth-summary">
                    <div><span>Credenciales</span><strong>{selected.authRef ? 'SecretRef configurado' : 'Sin credenciales'}</strong></div>
                    <div><span>Gateway</span><strong>{selected.kind === 'internal' ? 'No requerido' : 'Requiere gateway si usa secretos'}</strong></div>
                  </div>
                </section>

                <section className="ec-data-source-section" aria-labelledby="ec-data-source-schema">
                  <h4 id="ec-data-source-schema">Esquema</h4>
                  <p>Política: <strong>{selected.schemaDiscovery}</strong></p>
                  <Button
                    size="sm"
                    variant="outline"
                ...
                <section className="ec-data-source-section" aria-labelledby="ec-data-source-test">
                  ...
                  {adapterRegistered && compatibility.length > 0 ? <small>Prueba deshabilitada: resuelve primero los avisos de compatibilidad.</small> : null}
                  {adapterRegistered && !environmentEnabled ? <small>Prueba deshabilitada: el entorno no pertenece al scope de la fuente.</small> : null}
                </section>
              </>
      ...
      <Sheet open={inspectorOpen} onOpenChange={setInspectorOpen}>
        <SheetContent side="right" className="ec-data-source-inspector-sheet">
          ...
          {selected ? <InspectorContent source={selected} adapterRegistered={adapterRegistered} compatibility={compatibility} /> : null}
        </SheetContent>
      </Sheet>
    </section>
  );
