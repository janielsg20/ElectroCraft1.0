...
test('native runtime and native preview do not depend on DOM/editor packages', () => {
  ...
  for (const name of ['@electrocraft/runtime-native', '@electrocraft/native-preview']) {
    for (const dep of name === '@electrocraft/runtime-native' ? snapshot.boundaries.packages[name] : snapshot.boundaries.apps[name]) {
...
test('Spanish architecture help descriptor is registered as a fixture', () => {
  const descriptor = JSON.parse(fs.readFileSync(path.join(root, 'tooling/fixtures/help.architecture.repository.json'), 'utf8'));
