...
## Por qué existe
Esta microfase existe para **Binding/query/form/action capabilities** dentro de la responsabilidad de `F20`: analizar compatibilidad multi-target. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.
...
`Compatibilidad` consumes declarations; source editors show local diagnostics.

## Diseño visual exacto
Badges/warnings contextual only.
...
1. Bindings declare target support based on source/runtime.
2. Queries declare compiler capability by Data Source and target.
3. Forms declare validation/submit/upload mapping per target.
4. Actions declare runtime/compiler support per target.
5. Auth/permissions declare enforcement target.
6. Static may block mutable/session/server features; must explain exact reason.
7. LAMP declares PHP/PDO/server adapters.
8. WordPress declares native WP APIs/REST/capability adapters.
9. Capacitor declares Web runtime + plugin adaptations.
10. Local target stores definitions without runtime execution.
11. Test a mixed app with internal/external data, auth, form and ActionGraph across all profiles.
12. Actualiza Capability/TargetDescriptor contract.
13. Analiza todos los nueve targets desde el mismo project snapshot.
14. Renderiza Compatibility/Export UI desde registries, no arrays hardcoded divergentes.
15. Bloquea Generate solo por blockers reales.
16. Prueba target parity/status/config host.
