...
No Studio screen own; diagnostics appear in Compatibilidad.

## Diseño visual exacto
No compiler UI; target adaptation shown as diagnostic.
...
1. Define `ActionCompilerPort`/runtime boundary independiente de Rete classes.
2. Web target compila/ejecuta semantics mediante Action runtime/rete-engine solo cuando corresponda.
3. Native target mapea Action nodes a Expo/Native adapters.
4. Data/Navigation/Auth nodes se resuelven por ports, no imports directos de target.
5. Los compilers LAMP, WordPress y Capacitor se implementan en sus fases Core F24–F26 y deben consumir este mismo Action compiler boundary.
6. Test same ActionGraph against Web and Native compiler/runtime fixtures.
7. Actualiza ActionGraph/node definition.
8. Mapea a Rete editor/runtime sin persistir classes.
9. Implementa node service/provider adapter.
10. Define target compiler semantics.
11. Prueba graph/history/runtime/loop/error.
