...
## Por qué existe
Esta microfase existe para **Component/style capabilities** dentro de la responsabilidad de `F20`: analizar compatibilidad multi-target. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.
...
`Publicar > Compatibilidad` later consumes; Editor may show small capability badge only when relevant.

## Diseño visual exacto
No duplicate compatibility screen in Editor.
...
1. Cada Core ComponentDefinition declara soporte de los nueve targets mediante defaults + explicit exceptions.
2. Layout/style properties declaran exact/adapted/blocked per target.
3. `Embed/Iframe`: Web/Static/PWA/LAMP/WordPress/Capacitor may support; Expo Native may adapt/block based on WebView policy.
4. Table may map to Native cards but remain HTML table on Web/LAMP/WordPress when semantic.
5. WordPress mapping can be native block/pattern/dynamic block; this is target metadata, not another component definition.
6. LAMP maps semantic components to HTML/PHP renderer.
7. Local target stores exact canonical definition.
8. Diagnostics name exact node/property and proposed adaptation.
9. Test representative layout/content/navigation/media components across all targets.
10. Actualiza Capability/TargetDescriptor contract.
11. Analiza todos los nueve targets desde el mismo project snapshot.
12. Renderiza Compatibility/Export UI desde registries, no arrays hardcoded divergentes.
13. Bloquea Generate solo por blockers reales.
14. Prueba target parity/status/config host.
