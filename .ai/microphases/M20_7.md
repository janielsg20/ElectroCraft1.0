...
## Por qué existe
Esta microfase existe para **Capability regression fixtures** dentro de la responsabilidad de `F20`: analizar compatibilidad multi-target. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.
...
No new UI; audits Compatibilidad.

## Diseño visual exacto
Regression matrix fixtures across representative artifacts.
...
6. Actualiza Capability/TargetDescriptor contract.
7. Analiza todos los nueve targets desde el mismo project snapshot.
8. Renderiza Compatibility/Export UI desde registries, no arrays hardcoded divergentes.
9. Bloquea Generate solo por blockers reales.
10. Prueba target parity/status/config host.
