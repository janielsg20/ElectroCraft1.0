...
1. Crea la fixture canónica inicial `Reserva Studio` usando exclusivamente APIs/servicios públicos ElectroCraft.
2. Incluye Screens, Navigation, Internal Data, REST mock source, Relations, Query/Filter, State, Form, Administration, Roles/Auth, ActionGraph, Media y Theme.
3. Incluye al menos una capability de dispositivo compatible con los targets Native/Capacitor y diagnosticada para server/Web targets.
4. Incluye un reusable block aplicado mediante AI Draft/Apply usando mock determinista.
5. No añadas hacks target-specific al ProjectDefinition.
6. Guarda una revision y checksum base para todas las pruebas de paridad posteriores.
7. Ejecuta reopen/recovery para comprobar que la fixture no depende de transient engine state.
8. Ejecuta Preview Web/Admin/Native smoke.
9. Congela la fixture versionada.
10. Documenta expected capabilities y expected adaptations por target antes de exportar.
