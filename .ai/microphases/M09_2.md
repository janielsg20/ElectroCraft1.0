...
## Por qué existe
Esta microfase existe para **ElectroCraftQueryDefinition y RQB payload adapter** dentro de la responsabilidad de `F09`: resolver bindings/queries/listings/facets/cache. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **ElectroCraftQueryDefinition y RQB payload adapter** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
`Datos > Consultas > Nueva/Editar`.

## Diseño visual exacto
List 280 izquierda; central Fuente/Condiciones/Orden/Paginación/Resultado; diagnostics summary.
...
1. `ElectroCraftQueryDefinition` guarda source(s), combine mode, sort/pagination/aggregate/cache policy y un wrapper de rules RQB versionado.
...
8. Actualiza Query/Binding portable definition.
9. Usa RQB solo para condition authoring.
10. Compila a DataSource operation; TanStack Query posee cache en JS runtime.
11. Conecta Listing/Filter/Inspector.
12. Prueba types, invalid rule fail-closed, cache/invalidation.
