...
1. Usa la misma revision ExportIR que los demás targets.
2. Genera Android Expo source y verifica prebuild/build según toolchain real.
3. Genera iOS Expo source y verifica prebuild/build en entorno permitido.
4. Prueba Navigation/Routes/Deep Links, Data, State, Forms, Actions, Auth, Media y permisos.
5. Verifica Native-specific adaptations contra Compatibility report.
6. Verifica no DOM/Web-only dependencies leak into Native packages.
7. Verifica artifacts reales y checksums cuando APK/AAB/IPA existan.
8. Registra skips de toolchain como evidencia explícita, nunca fake pass.
9. Compara semantic snapshot Android vs iOS.
10. Cierra ambos targets.
