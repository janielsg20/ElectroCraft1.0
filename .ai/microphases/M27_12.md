...
## Por qué existe
Capacitor, LAMP y WordPress son targets Core de primera clase. Esta microfase demuestra su paridad mediante el mismo contrato de exportación/evidencia que Web y Expo.
...
No crea UI nueva. Valida `Publicar > Compatibilidad` y `Publicar > Exportar` para Capacitor, LAMP y WordPress.
...
Paridad; Capacitor; LAMP; WordPress; Verificado; Adaptaciones; Bloqueado; Artifact; Informe.
...
1. Usa exactamente el mismo revision checksum/ExportIR de la fixture final.
2. Capacitor: Web build -> cap sync -> source/native smoke -> plugin/capability checks.
3. LAMP: Composer install -> MySQL/MariaDB migration -> HTTP/forms/auth/admin/security E2E.
4. WordPress: clean wp-env -> install Theme + Companion Plugin -> activation/migrations -> front/admin/forms/REST/RBAC/lifecycle E2E.
5. Compara cada target contra ExportParityMatrix.
6. Verifica que ningún target use label `opcional`, `fallback`, `legacy` o `secondary`.
7. Verifica que compatibility blockers aparecen antes de Generate.
8. Verifica que generated source no contiene Studio secrets/history.
9. Verifica artifacts reales/checksums.
10. Registra exact/adapted/blocked differences con source references.
11. Falla la microfase si LAMP o WordPress carece de clean-install fixture.
12. Falla la microfase si Capacitor solo genera config sin sync/source verification.
