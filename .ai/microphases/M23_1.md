...
## Por qué existe
Esta microfase existe para **Native permissions compiler** dentro de la responsabilidad de `F23`: mapear device capabilities. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.
...
`Publicar > Exportar > Android/iOS > Permisos` + relevant `Compatibilidad` diagnostics.

## Diseño visual exacto
Permission summary list capability -> platform permission -> reason; no permission not used.
