...
## Por qué existe
Esta microfase existe para **Sincronizar Puck actions con ElectroCraftDocument** dentro de la responsabilidad de `F05`: integrar Puck sin duplicar su editor. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Sincronizar Puck actions con ElectroCraftDocument** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
No añade control independiente; sincronización detrás de `Construir > Editor`.

## Diseño visual exacto
Feedback únicamente save/status/selection; no mostrar eventos internos.
...
2. Después de una acción estable, transforma a ElectroCraftDocument.
3. Debounce autosave por F04.
4. No necesitas MapOperations->universal CommandBus.
5. Prueba reorder/edit/duplicate/delete.
6. Actualiza mapping ElectroCraftDocument <-> Puck projection.
7. Usa Puck public Composition/Slots/Fields/Outline/Preview en vez de recrearlos.
8. Conecta changes a canonical commands/repository.
9. Mantén history de editor separado de revisions.
10. Prueba round-trip/nesting/selection/history.
