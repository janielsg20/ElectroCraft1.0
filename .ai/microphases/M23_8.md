...
## Por qué existe
Esta microfase existe para **Android exporter y build** dentro de la responsabilidad de `F23`: generar y validar Android/iOS con Expo. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.
...
Source/APK/AAB status cards; toolchain/signing diagnostics; no fake artifact links.
...
1. Genera proyecto Expo/Android source desde validated ExportIR.
2. Ejecuta install/typecheck/prebuild Android donde corresponda.
3. Si Android toolchain disponible, produce y verifica APK debug.
4. AAB release solo si signing/config real está disponible.
5. Nunca inventes path de APK/AAB: stat/checksum obligatorio.
6. Reporta permisos/modules incluidos.
7. Test no-artifact state when toolchain unavailable.
