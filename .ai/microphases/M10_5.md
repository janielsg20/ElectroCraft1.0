...
No Studio screen nueva; output visible en Vista previa/exports.

## Diseño visual exacto
Read-only render must match editor semantics without editor chrome.
...
1. Persiste Tiptap JSON bajo wrapper/version portable.
2. Define allowed extension profile compartido por editor/renderers.
3. Web read-only usa `@tiptap/static-renderer` o helper oficial apropiado; no parser HTML propio.
4. Native renderer mapea el subset soportado a Native richtext/text presentation y emite diagnostics por marks/nodes no soportados.
5. Optional server target packs documentarán su propia pre-render strategy fuera del Core release.
6. Prueba JSON round-trip, Web static render, Native supported subset y unsupported node diagnostic.
7. Define portable media/rich-text payload.
8. Implementa MediaBlobStore o Tiptap adapter; no segundo engine.
9. Conecta Studio UI/field.
10. Implementa target-safe read rendering.
11. Prueba storage/security/round-trip.
