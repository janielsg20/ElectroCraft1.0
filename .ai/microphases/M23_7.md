...
## Por qué existe
Esta microfase existe para **Expo export profile** dentro de la responsabilidad de `F23`: generar y validar Android/iOS con Expo. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.
...
`Publicar > Exportar > Android/iOS`.

## Diseño visual exacto
Mobile target options app ID/version/orientation/features; capability summary.
...
1. Define Expo export profile con app id/bundle identifier, version, icons, splash, orientation, permissions y environment refs.
2. Deriva modules/permisos de PlatformCapabilityRegistry, no de una checklist manual.
3. Genera `app.json/app.config` según versión Expo pinned.
4. Source export siempre puede producirse si compatibility está verde.
5. Build status distingue Source ready / Build environment missing / Build succeeded / Signing required.
6. No incluye Capacitor dentro del perfil Expo; Capacitor es un target Core separado implementado en F24.
7. Prueba profile minimal y full capabilities.
