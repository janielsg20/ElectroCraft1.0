...
2. Añade `applyMode`: `live`, `manual`, `url` según target/runtime capabilities.
3. `FilterState` es runtime state; no modifica QueryDefinition.
4. Soporta applied chips, clear-one, clear-all y Reset.
5. Facets/counts sobre Internal Data usan GenericFieldIndexer cuando exista; otras sources dependen de source capabilities/query.
6. Taxonomy filter soporta hierarchy para Internal Data Models.
7. Pagination/Load More consume el mismo Query/FilterState.
8. Cada target Core declara sus transport/apply modes: Web/Native/Capacitor/LAMP/WordPress; las diferencias se expresan como capabilities/adaptations.
9. Test aliases/facets/chips/reset/Web URL mode/Native adaptation.
10. Actualiza Query/Binding portable definition.
11. Usa RQB solo para condition authoring.
12. Compila a DataSource operation; TanStack Query posee cache en JS runtime.
13. Conecta Listing/Filter/Inspector.
14. Prueba types, invalid rule fail-closed, cache/invalidation.
