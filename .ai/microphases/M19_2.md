...
## Por qué existe
Esta microfase existe para **Admin runtime preview** dentro de la responsabilidad de `F19`: previsualizar/diagnosticar runtime. Debe cerrar una capacidad concreta y no abrir un subsystem paralelo.

## Resultado exacto
Al cerrar esta microfase, **Admin runtime preview** debe existir como capacidad real, integrada y verificable. No se acepta placeholder ni una implementación paralela al engine.
...
`Publicar > Vista previa > Administración`.

## Diseño visual exacto
Same preview shell targeting generated admin; role simulation banner.
...
1. Renderiza AdminScreen/Administración documents con Refine runtime.
2. Administración theme real.
3. Role/permission checks.
4. DataView mobile/desktop modes.
5. No Puck selection overlays.
6. Ejecuta runtime real, no screenshot/mock.
7. Conecta route/role/platform selectors.
8. Instrumenta traces sin exponer secrets.
9. Aísla effects de Preview cuando sea necesario.
10. Prueba Web/Admin/Native modes y Debug.
