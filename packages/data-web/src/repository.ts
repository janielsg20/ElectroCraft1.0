...
export function createDrizzleProjectRepository(db: StudioProjectDatabase) {
  return Object.freeze({
    async saveProject(request: NormalizedSaveProjectRequest): Promise<ProjectStorageRevision> {
    ...
    async openProject(projectId: string): Promise<OpenProjectResult | null> {
    ...
    async verifyProject(projectId: string): Promise<ProjectIntegrityReport> {
      const opened = await this.openProject(projectId);
      if (!opened) {
        return Object.freeze({
          projectId,
          coherent: false,
