 ...
 * the public onAction snapshots and reacts when canonical authoring Data has
 * actually changed. UI-only selection/viewport actions retain the same Data
 * reference and therefore never enter project persistence.
 */
export function resolvePuckDocumentActionChange(
  ...
  if (appState.data === prevAppState.data) return null;
